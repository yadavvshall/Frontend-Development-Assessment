...
var page_14 = ["M1L0T1P14_1", "M1L0T1P14_2", "M1L0T1P14_3"];
var page_32 = [ "M1L0T1P32_4", "M1L0T1P32_3", "M1L0T1P32_2","M1L0T1P32_1"];
...
function pauseVideoOnPopup() {

    try {
        if ($("#myVideo")[0] != undefined) {
            $("#myVideo")[0].pause();
        }
    } catch (error) {
        console.log(error)
...
// function jumpToNextPageOfMulti(page_array, nextPage){
...
function fnLoadPage(lModNum, lLessNum, lTopNum, lPageNum) {
    ...
    $('#contentFrame').load(currPgSrc, function() {
        ...
        // alert("test 1234 == "+currPgSrc);
        console.log("Page name ====="+currPgSrc)
...
function fnTools() {
    if (isToolEnable) {
        ...
        $(".ClsToolPop").css("top", "25px").css({ "transform": "translateY(0%)" });
        $(".ClsToolbtn ").addClass("ClsToolbtnSelected");
        //$(".ClsToolPop,.ClsToolSubPop").css("display","none");
        isToolEnable = false;
    } else {
        $(".ClsToolPop").css("top", "0px").css({ "transform": "translateY(100%)" });
        $(".ClsToolbtn ").removeClass("ClsToolbtnSelected");
